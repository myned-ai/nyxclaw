...
let avatarHttpModulePromise: Promise<typeof import("./avatar-http.js")> | undefined;
...
function getAvatarHttpModule() {
  avatarHttpModulePromise ??= import("./avatar-http.js");
  return avatarHttpModulePromise;
}
...
function isAvatarHttpPath(pathname: string): boolean {
  return pathname === "/v1/chat/completions/avatar";
}
...
export function createGatewayHttpServer(opts: {
  ...
  async function handleRequest(req: IncomingMessage, res: ServerResponse) {
    ...
    try {
      ...
      if (openAiChatCompletionsEnabled && isAvatarHttpPath(scopedRequestPath)) {
        requestStages.push({
          name: "avatar",
          run: async () =>
            (await getAvatarHttpModule()).handleAvatarHttpRequest(req, res, {
              auth: resolvedAuth,
