...
impl AgentBuilder {
    ...
    pub fn response_format(mut self, response_format: Option<serde_json::Value>) -> Self {
        ...
        for _ in 0..self.config.max_tool_iterations {
            ...
            if let (Some(ref cache), Some(ref key)) = (&self.response_cache, &cache_key) {
                if let Ok(Some(cached)) = cache.get(key) {
                    ...
                    // Emit speech_chunk events for the cached response
            ...
            if calls.is_empty() {
                ...
                // Emit speech_chunk events for the final response
                for chunk in split_into_sentence_chunks(&final_text, 100) {
                    let _ = event_tx
                        .send(serde_json::json!({
                            "type": "speech_chunk",
                            "content": chunk
